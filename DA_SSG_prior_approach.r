...
#Marginal log likelihood to be maximized
negloglik_normal <- function(xpar, xx){
  
  loglik <- sum(
    log(
      (1-xpar[1])*dnorm(xx, mean = 0, sd = 1)+
        (xpar[1])*dnorm(xx, mean = 0, sd = sqrt(xpar[2]+1))
    )
  )
  return(-loglik)
}
...
wpos_normal <- function(w, xx, tau2){
  wpostt <- (w*dnorm(xx, sd = sqrt(1+tau2)))/(w*dnorm(xx,
...
tau2_mix_gaussian <- rep_len(100, aux_data_size)

# Defining the optimization intervals 
startpar  <-  c(0.5,0.5)
lopar     <-  c(0,0.01)
hipar     <-  c(1,0.70)
...
# ========================== Generating mix_var ================================    
  ...
  for (jj in nlevelsWT(wav_lat_var):1){
    ...
    parameters <- optim(startpar, negloglik_normal, method="L-BFGS-B", 
                        lower = lopar, upper = hipar, 
                        xx = accessD.wd(wav_lat_var,level=(jj-1)))$par
    
    pi_mix_wavelet[(2+r):(r+q+1)] <- rep_len(parameters[1],q)
    
    tau2_mix_gaussian[(2+r):(r+q+1)] <- rep_len(parameters[2],q)
